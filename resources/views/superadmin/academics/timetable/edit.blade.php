            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{route('superadmin.timetable')}}" class="text-decoration-none">Time Table</a></li>
                ...
                <div class="border border-3 p-4 rounded">
                    <div class="card">
                        <div class="card-body p-4">
                            <div class="row g-3">
                                <div class="form-body">
                                    <form class="row g-3" method="POST" action="{{ route('superadmin.timetable.update',$timeTable->id) }}">
                                        @csrf
                                        <div class="col-6">
                                            ...
                                            <select class="form-control @error('class') is-invalid @enderror" name="class" id="class">
                                                ...
                                                <option value="{{$class->class}}" @if(old('class',$timeTable->class)==$class->class) selected @endif>
                                        ...
                                        <div class="col-6">
                                            ...
                                            <input type="time" value="{{old('start_time',$timeTable->start_time)}}" class="form-control @error('start_time') is-invalid @enderror" name="start_time" id="start_time">
                                            @error('start_time') <span class="text-danger">{{$message}}</span> @enderror
                                        </div>
                                        <div class="col-6">
                                            <label for="end_time" class="form-label">End Time</label>
                                            <input type="time" value="{{old('end_time',$timeTable->end_time)}}" class="form-control @error('end_time') is-invalid @enderror" name="end_time" id="end_time">
                                        ...
                                        <div class="col-6">
                                            ...
                                            <select class="form-control @error('teacher') is-invalid @enderror" name="teacher">
                                                ...
                                                <option value="{{$teacher->id}}" @if(old('teacher',$timeTable->teacher)==$teacher->id) selected @endif>{{$teacher->name}}</option>
                                                @endforeach
                                            </select>
...
<script>
    ...
    let selectedGroup = @json($timeTable->subject_group);
    let selectedSection = @json($timeTable->section);
    let selectedSubject = @json($timeTable->subject);
