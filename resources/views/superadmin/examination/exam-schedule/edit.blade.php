            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{route('superadmin.examination.exam.schedule')}}" class="text-decoration-none">Exam Schedule</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Edit Exam Schedule</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="border border-3 p-4 rounded">
                    <div class="card">
                        <div class="card-body p-4">
                            <div class="row g-3">
                                <div class="form-body">
                                    <form class="row g-3" method="POST" action="{{ route('superadmin.examination.exam.schedule.update',$schedule->id) }}">
                                        @csrf
                                        <div class="col-12">
                                            ...
                                            <select class="form-control @error('exam_type') is-invalid @enderror" name="exam_type">
                                                ...
                                                <option value="{{$type->id}}" @if(old('exam_type',$schedule->exam_type)==$type->id) selected @endif>{{$type->exam_name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-6">
                                            ...
                                            <select class="form-control @error('class') is-invalid @enderror" name="class" id="class">
                                                ...
                                                <option value="{{$class->class}}" @if(old('class',$schedule->class)==$class->class) selected @endif>
                                        ...
                                        <div class="col-6">
                                            ...
                                            <input type="date" value="{{old('date',$schedule->date)}}" class="form-control @error('date') is-invalid @enderror" name="date">
                                            @error('date') <span class="text-danger">{{$message}}</span> @enderror
                                        </div>
                                        <div class="col-6">
                                            <label for="start_time" class="form-label">Start Time</label>
                                            <input type="time" value="{{old('start_time',$schedule->start_time)}}" class="form-control @error('start_time') is-invalid @enderror" name="start_time" id="start_time">
                                            @error('start_time') <span class="text-danger">{{$message}}</span> @enderror
                                        </div>
                                        <div class="col-6">
                                            <label for="end_time" class="form-label">End Time</label>
                                            <input type="time" value="{{old('end_time',$schedule->end_time)}}" class="form-control @error('end_time') is-invalid @enderror" name="end_time" id="end_time">
                                        ...
                                        <div class="col-6">
                                            ...
                                            <select class="form-control @error('teacher') is-invalid @enderror" name="teacher">
                                                ...
                                                <option value="{{$teacher->id}}" @if(old('teacher',$schedule->teacher)==$teacher->id) selected @endif>{{$teacher->name}}</option>
                                                @endforeach
                                            </select>
...
<script>
    ...
    let selectedGroup = @json($schedule->subject_group);
    let selectedSection = @json($schedule->section);
    let selectedSubject = @json($schedule->subject);
