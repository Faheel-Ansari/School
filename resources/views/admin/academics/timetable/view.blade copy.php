    ...
    <div class="page-content">
        ...
        <div class="card-body">
            ...
            <div class="mb-3 d-flex align-items-center justify-content-end">
                <a href="{{route('superadmin.timetable.add')}}" class="btn btn-secondary"><i class="fa-solid fa-plus"></i> Add</a>
            </div>
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form class="row g-3 mb-4" action="{{ route('superadmin.timetable.view') }}">
                            <div class="col-5">
                                <label for="class" class="form-label">Class</label>
                                <select class="form-control @error('class') is-invalid @enderror" name="class" id="class">
                                    <option selected disabled>-- Select Class --</option>
                                    @foreach($classes as $class)
                                    <option value="{{$class->id}}" @if(old('class',$selectedClass)==$class->id) selected @endif>{{$class->class}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="table-responsive">
                            <table id="viewDataTable" class="table mb-0">
                                ...
                                <tbody>
                                    ...
                                    @php
                                    $classTimeTables = $timeTables->where('class',$class->id)->where('section',$section->id);
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <td style="vertical-align: middle">
                                            <a href="{{route('superadmin.timetable.destroy',$ids)}}" class="btn btn-danger "><i class="fa-solid fa-trash"></i>Delete this Time Table</a>
                                        </td>
                                    </tr>
                                    @endif
                                    @endforeach
    ...
    <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="{{ 'timeTableModalLabel' . $class->id . '_' . $section->id }}">Time Table - Class {{ $class->class }} / Section {{ $section->section }}</h1>
            ...
            <div class="modal-body">
                <div class="d-flex justify-content-around">
                    ...
                    $dayTimeTables = $timeTables->where('class', $class->id)->where('section', $section->id)->where('day', $day);
                    @endphp
                    ...
                    <div class="text-center">
                        ...
                        <div class="d-flex flex-column mb-2">
                            <div class="@if($idx+1==1) bg-warning-subtle @elseif($idx+1==2) bg-danger-subtle @elseif($idx+1==3) bg-primary-subtle @elseif($idx+1==4) bg-secondary-subtle @elseif($idx+1==5) bg-success-subtle @elseif($idx+1==6) bg-info-subtle @elseif($idx+1==7) bg-dark-subtle @endif p-2 rounded-top">
                                ...
                                <p class="mb-0">{{ $timetable->topic }}</p>
                                <p class="mb-0"><strong>{{ $timetable->name }}</strong></p>
                            </div>
                            <div class="d-flex bg-danger justify-content-around @if($idx+1==1) bg-warning-subtle @elseif($idx+1==2) bg-danger-subtle @elseif($idx+1==3) bg-primary-subtle @elseif($idx+1==4) bg-secondary-subtle @elseif($idx+1==5) bg-success-subtle @elseif($idx+1==6) bg-info-subtle @elseif($idx+1==7) bg-dark-subtle @endif p-2 pt-0 rounded-bottom">
                                <a href="{{route('superadmin.timetable.edit',$timetable->id)}}" class="text-primary fs-5"><i class="fa-solid fa-pen-to-square"></i></a>
                                <a href="{{route('superadmin.timetable.destroy',$timetable->id)}}" class="text-danger fs-5"><i class="fa-solid fa-trash"></i></a>
                            </div>
                        </div>
                        @endforeach
                        @endif
