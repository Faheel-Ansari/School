                                ...
                                <div class="">
                                    ...
                                    <div class="form-body">
                                        ...
                                        <div class="text-center mb-4 text-danger">
                                            <p>Your account has been <strong>suspended</strong> due to {{session('message')}}. Please contact to admin.</p>
                                        </div>
                                        @elseif (session('adminStatus') == 'adminSuspend')
                                        <div class="text-center mb-4 text-danger">
                                            <p>Your account has been <strong>suspended</strong>. Please contact to admin.</p>
                                        </div>
                                        @endif
                                        @if (session('user') == 'no-user' || session('login') == 'invalid')
                                        <div class="text-center mb-4 text-danger">
                                            <p><strong>Invalid CNIC or Password</strong></p>
                                        </div>
                                        @endif
                                        <form class="row g-3" name="" id="" method="POST" action="{{ route('parent.login.store') }}">
                                            @csrf

                                            <div class="col-12">
                                                <label for="cnic" class="form-label">CNIC No</label>
                                                <input type="text" class="form-control @error('cnic') is-invalid @enderror" name="cnic" id="cnic" value="{{old('cnic')}}" placeholder="eg: 42101-1234567-8">
                                                @error('cnic')
